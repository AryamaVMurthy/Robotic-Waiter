...
DISCRETE_ACTION_DURATION = 0.15
POSE_UPDATE_WAIT = 2.5
# --- Configuration (Mirrors CTE Server, including tuned values) ---
# Network
SERVER_IP = '192.168.214.19' # Your PC's IP
SERVER_PORT = 5683
ESP32_IP = '192.168.137.163' # Set ESP32 IP directly
...
MOTOR_TURN_SPEED = 255    # Default turning speed (Synced with CTE)
...
WAYPOINT_RADIUS = 0.050 # Meters (4.0 cm) - Keep increased radius for delay tolerance
...
async def control_loop(client_context):
    ...
    print("\n===== MODIFIED DISCRETE WAYPOINT CONTROLLER =====")
    print("- Using only FINAL DESTINATION as waypoint")
    print("- Providing detailed pose and navigation debug info")
    print("=================================================\n")
    ...
    # Print destination waypoint info (only one waypoint - the final destination)
    if current_path_real and len(current_path_real) > 0:
        dest_waypoint = current_path_real[0]  # There's only one waypoint now
        print(f"\n🎯 DESTINATION WAYPOINT: ({dest_waypoint[0]:.4f}, {dest_waypoint[1]:.4f}) meters\n")

    instruction_counter = 0  # For tracking new instructions
    ...
    while True:
        ...
        if current_pose is None:
            ...
            if controller_state != "POSE_LOST":
                 print("\n❌ POSE LOST - Robot position could not be detected!")
            ...
            if controller_state != "FINISHED":
                 print("\n🏁 DESTINATION REACHED - Path complete!")
            ...
            # Print detailed waypoint info
            print(f"\n🎯 TARGET WAYPOINT: ({target_waypoint[0]:.4f}, {target_waypoint[1]:.4f}) meters")
            print(f"   Distance to target: {dist_to_wp:.4f} meters")
            print(f"   Heading to target: {math.degrees(heading_to_target):.2f}° ({heading_to_target:.4f} rad)")
            print(f"   Heading error: {math.degrees(heading_error):.2f}° ({heading_error:.4f} rad)")
            print(f"   Current state: {controller_state}")
            print(f"   Waypoint radius threshold: {WAYPOINT_RADIUS:.4f} meters")
            print(f"   Angle threshold: {math.degrees(ANGLE_THRESHOLD):.2f}° ({ANGLE_THRESHOLD:.4f} rad)")
            ...
            if controller_state == "IDLE":
                if abs(heading_error) > ANGLE_THRESHOLD:
                    ...
                    print(f"\n🔄 IDLE->ALIGNING: Needs turn (Error: {math.degrees(heading_error):.2f}°)")
                    ...
                    print(f"\n⬆ IDLE->MOVING: Aligned within threshold ({math.degrees(ANGLE_THRESHOLD):.2f}°)")
                    print(f"   Selected action: {next_action}")
                ...
                if abs(heading_error) < ANGLE_THRESHOLD:
                    ...
                    print(f"\n✅ ALIGNING->IDLE: Alignment sufficient (Error: {math.degrees(heading_error):.2f}°)")
                    print(f"   Selected action: {next_action}")
                    controller_state = "IDLE" # Go back to IDLE to decide next (likely MOVE)
                else:
                    # Continue aligning with another discrete turn
                    next_action = "TURN_RIGHT" if heading_error < 0 else "TURN_LEFT"
                    print(f"\n🔄 ALIGNING: Continue turn (Error: {math.degrees(heading_error):.2f}°)")
                    print(f"   Selected action: {next_action}")
                ...
                if dist_to_wp < WAYPOINT_RADIUS:
                    # Waypoint reached
                    print(f"\n🏁 MOVING->IDLE: Waypoint reached! (Distance: {dist_to_wp:.4f}m)")
                    print(f"   Selected action: STOP")
                    next_action = "STOP"
                    current_waypoint_index += 1
                    if current_waypoint_index >= len(current_path_real):
                        print("\n🎉 FINAL DESTINATION REACHED! Path Complete.")
                        current_path_real = None # Clear path
                        current_waypoint_index = -1
                        controller_state = "FINISHED" # Transition to FINISHED state
                    else:
                        # This should never happen with only one waypoint
                        print(f"⚠ WARNING: Multiple waypoints detected! Proceeding to waypoint {current_waypoint_index}")
                        controller_state = "IDLE"
                else:
                    # Waypoint not reached, check alignment while moving
                    if abs(heading_error) > ANGLE_THRESHOLD * 1.5: # Drifted significantly
                        print(f"\n🛑 MOVING->IDLE: Angle drifted (Error: {math.degrees(heading_error):.2f}°)")
                        print(f"   Need to stop and realign. Selected action: STOP")
                        next_action = "STOP"
                        controller_state = "IDLE" # Go back to IDLE to force realignment turn
                    else:
                        # Continue moving forward with another discrete step
                        next_action = "MOVE_FORWARD"
                        print(f"\n⬆ MOVING: Continue forward (Distance: {dist_to_wp:.4f}m)")
                        print(f"   Heading error: {math.degrees(heading_error):.2f}° (within {math.degrees(ANGLE_THRESHOLD*1.5):.2f}° threshold)")
                        print(f"   Selected action: {next_action}")
                 ...
                 print(f"\n⚠ Robot in {controller_state} state. Selected action: STOP")
        ...
        # Print instruction counter and separator to make new instructions stand out
        instruction_counter += 1
        print(f"\n{'='*30}")
        print(f"INSTRUCTION #{instruction_counter}: {next_action}")
        print(f"{'='*30}")
        ...
        if next_action == "MOVE_FORWARD":
            ...
            print(f"  🤖 Robot moving at speed {MOTOR_FORWARD_SPEED}")
            ...
            direction = "LEFT ⬅" if next_action == "TURN_LEFT" else "RIGHT ➡"
            print(f"  📲 Executing {next_action} (Seq: {command_seq}) for {DISCRETE_ACTION_DURATION}s...")
            print(f"  🤖 Robot turning {direction} at speed {MOTOR_TURN_SPEED}")
            asyncio.create_task(send_coap_command(client_context, ESP32_IP, cmd_data, command_seq))
            await asyncio.sleep(DISCRETE_ACTION_DURATION)
            action_executed = True
        elif next_action == "STOP":
             print(f"  📲 Executing STOP (Seq: {command_seq}) based on state: {controller_state}")
             print(f"  🤖 Robot stopping")
             # No sleep needed here as STOP is immediate

        # --- Always send STOP after a MOVE/TURN action or if STOP was the decided action ---
        # Use a new sequence number for the explicit STOP command
        command_seq += 1
        stop_cmd_data = {"cmd": "STOP", "seq": command_seq}
        if action_executed:
            print(f"  📲 Sending STOP (Seq: {command_seq}) after discrete action")
        # Send the STOP command regardless (ensures robot stops between steps)
        asyncio.create_task(send_coap_command(client_context, ESP32_IP, stop_cmd_data, command_seq))

        # --- Wait for Pose Update ---
        # Only wait if the robot is actively navigating (not finished or lost)
        if controller_state not in ["FINISHED", "POSE_LOST"]:
            print(f"  ⏱ Waiting {POSE_UPDATE_WAIT}s for pose update...")
            await asyncio.sleep(POSE_UPDATE_WAIT)
        else:
            # If finished or lost, a shorter delay might be okay, or just rely on the loop bottom delay
            await asyncio.sleep(0.1) # Short delay in terminal states
...
async def main():
    ...
    try:
        # Store the full path for visualization purposes
        full_planned_path_map_indices = planned_path_map_indices.copy()
        
        # Extract just the final destination for control (single waypoint approach)
        final_destination = planned_path_map_indices[-1]
        
        # Create a path with just the final destination
        current_path_real = []
        rx, ry = map_indices_to_real(final_destination[0], final_destination[1])
        if rx is None: raise ValueError("Map indices to real conversion failed for final destination.")
        current_path_real.append((rx, ry))
        
        print(f"USING SINGLE WAYPOINT APPROACH:")
        print(f"  • Full A* path has {len(planned_path_map_indices)} points (used for visualization)")
        print(f"  • Control path contains only final destination: {current_path_real[0]} (meters)")
        print(f"  • Start point was at map indices: {start_map_indices_int}")
        print(f"  • Goal is at map indices: {goal_map_indices}")
