...
import sys
import traceback
from aiocoap import Context, Message, PUT, CHANGED
...
WAYPOINT_RADIUS = 0.15 # Meters (4.0 cm) - Keep increased radius for delay tolerance
...
class StatusResource(Resource):
    ...
    async def render_put(self, request):
        print("Received status PUT:", request.payload)
        if self.done_queue:
            await self.done_queue.put({"payload": request.payload})
...
async def control_loop(client_context):
    ...
    while True:
        ...
        status_res.done_queue = asyncio.Queue()
        ...
        if current_pose_raw is not None:
            ...
            real_coords = pixel_to_real(goal_xy_pixels[0], goal_xy_pixels[1])
        ...
        if DIR is not None:
            ...
            curr_task = asyncio.create_task(send_coap_command(client_context, ESP32_IP, cmd_data, command_seq))
            try:
                await asyncio.wait_for(curr_task, timeout=1.5)
            ...
            # else:
            #     print("ALREADY AT DESTINATION, NIGGA WE WON!!!")

        # wait for acknowledgement
        while True:
            item = await status_res.done_queue.get()
            # item is a dict: {"payload": raw_bytes}
            raw = item["payload"]
            try:
...
async def main():
    ...
    global status_res
    status_res = StatusResource()
    root = resource.Site(); root.add_resource(['status'], status_res)
